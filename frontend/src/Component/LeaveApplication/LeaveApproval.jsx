...
const mockRows = [
  { id: 236977, empId: 3016, empName: "MALLIKARJUNA RAO KOTA", unit: "UNIT-1", dept: "PRODUCTION", desg: "Supervisor",
    ...
    days: [
      ...
      { date: "2025-08-09", dayType: "FD", type: "", remarks: "" },
      { date: "2025-08-10", dayType: "FH", type: "", remarks: "" }
    ]
  },
  { id: 236981, empId: 61002, empName: "MAKIREDDI LOKESH", unit: "UNIT-6", dept: "PRODUCTION", desg: "Trainee Operator",
    ...
    days: [
      { date: "2025-08-08", dayType: "FD", type: "", remarks: "" },
      { date: "2025-08-09", dayType: "FD", type: "", remarks: "" },
...
export default function LeaveApprovalPage() {
 ...
 useEffect(() => {
  ...
  const columns = useMemo(() => [
    ...
    { field: "entry", headerName: "Entry Date", width: 160 },
    { field: "unit", headerName: "Unit", width: 90 },
    { field: "empName", headerName: "Employee Name", flex: 1, minWidth: 220 },
  ...
  const totalCLs = (sanction.days || []).reduce((total, d) => {
    if (d.type === "CL") {
      if (d.dayType === "FD") {
        total += 1;
      } else if (d.dayType === "FH" || d.dayType === "SH") {
        total += 0.5;
      }
    }
    return total;
...
//  }
  ...
  // };
    ...
    try {
  const res = await axios.post(`${import.meta.env.VITE_API_URL}/leave/approve`, {
    lno: selected.id,
    empid: selected.empId,
    frmdt: selected.from,
    todate: selected.to,
...
const generateDays = (from, to) => {
  const days = [];
  let current = new Date(from);
  const end = new Date(to);
  while (current <= end) {
    days.push({ date: current.toISOString().split("T")[0], dayType:"",type: "", remarks: "" });
      ...
      <Card sx={{ borderRadius: 2, boxShadow: "0 2px 12px rgba(16,24,40,0.06)", mb: 2 }}>
        <CardContent sx={{ pb: 1.5 }}>
          <Stack direction="row" spacing={2} flexWrap="wrap">
            ...
            <Button variant="contained" onClick={() => { /* optional fetch */ }}>
              Go
            </Button>
          </Stack>
...
>
    ...
    <>
      ...
      <Card variant="outlined" sx={{ mb: 2 }}>
        <CardContent sx={{ py: 1.5 }}>
          ...
          <GridRow label="Date" value={selected.entry} />
  ...
  <CardContent sx={{ py: 1.5 }}>
    ...
    <Stack spacing={1}>
      {(sanction.days || selected.days || generateDays(selected.from, selected.to)).map((day, index) => (
        <Stack direction="row" spacing={1.5} alignItems="center" key={index}>
          {/* Date */}
          <Typography sx={{ width: 80 }}>{day.date}</Typography>
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              setSanction((s) => {
                ...
                updated[index] = { ...updated[index], dayType: newDayType };
