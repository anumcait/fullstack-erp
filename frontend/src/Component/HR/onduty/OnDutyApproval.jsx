...
export default function OnDutyApprovalPage() {
  ...
  const fetchPendingOnDuty = async () => {
    try {
      ...
      console.error("Failed to load on duty apps", err);
    }
  };
  ...
  const onApprove = async () => {
    if (!selected || !selected.movement_id) {
      alert("No on duty application selected.");
      return;
    }

    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/onduty/approve`, {
      ...
      if (res.data.success || res.data.message) {
        showToast("On Duty application approved successfully ✅", "success");
  ...
  const onReject = async () => {
    ...
    try {
      ...
      if (res.data.success || res.data.message) {
        showToast("On Duty application rejected successfully", "success");
        await fetchPendingOnDuty();
        closeDrawer();
      } else {
    ...
    <Box sx={{ p: 1, bgcolor: "#f5f7fa", minHeight: "100vh" }}>
      ...
      <Card sx={{ borderRadius: 2, boxShadow: "0 2px 12px rgba(16,24,40,0.06)", mb: 2 }}>
        <CardContent sx={{ pb: 1.5 }}>
          <Stack direction="row" spacing={2} flexWrap="wrap">
            ...
            <Button variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={fetchPendingOnDuty}>
              Refresh
            </Button>
      ...
      <Card sx={{ borderRadius: 2, boxShadow: "0 2px 12px rgba(16,24,40,0.06)" }}>
        <Box sx={{ px: 2, py: 1.5, bgcolor: "#02AAB0", color: "#fff", borderTopLeftRadius: 8, borderTopRightRadius: 8 }}>
          <Typography fontWeight={700}>Pending On Duty Approval Details</Typography>
        </Box>
        <CardContent sx={{ pt: 1 }}>
          <div style={{ height: 520, width: "100%" }}>
            <DataGrid
              rows={dedupedRows}
              columns={columns}
              getRowId={(row) => `${row.movement_id}_${row.movement_date}`}
              ...
              <Card variant="outlined" sx={{ mb: 2 }}>
                <CardContent sx={{ py: 1.5 }}>
                  ...
                  <Stack direction="column" spacing={1}>
                    <Typography><b>Application No:</b> {selected.movement_id}</Typography>
                    <Typography><b>Date:</b> {formatDateDMYHM(selected.movement_date)}</Typography>
                    ...
                    <Typography><b>From Place:</b> {selected.from_place}</Typography>
                    <Typography><b>To Place:</b> {selected.to_place}</Typography>
                    <Typography><b>Purpose:</b> {selected.purpose}</Typography>
