...
export default function WoffApprovalPage() {
  ...
  const fetchPendingWoffs = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/woff/pending`);
  ...
  const columns = useMemo(() => [
    ...
    { field: "woff_id", headerName: "Woff App #", width: 160 },
    { field: "woff_date", headerName: "Date", width: 160 },
  ...
  const filtered = useMemo(() => {
    return rows.filter(r => {
      const inDate =
        (!filters.start || new Date(r.woff_date) >= new Date(filters.start)) &&
        (!filters.end || new Date(r.woff_date) <= new Date(filters.end));
      ...
      const matchQ = !q || [r.ename, r.unit, r.division, r.designation, r.reason].some(v => v.toLowerCase().includes(q));
      ...
      <Card sx={{ borderRadius: 2, boxShadow: "0 2px 12px rgba(16,24,40,0.06)", mb: 2 }}>
        <CardContent sx={{ pb: 1.5 }}>
          ...
          <Tabs value={tab} onChange={(_, v) => setTab(v)} textColor="primary" indicatorColor="primary" sx={{ mt: 2 }}>
            <Tab value="pending" label="Pending Woff Approval Details" />
            <Tab value="completed" label="Completed Woff Approval Details" />
            <Tab value="cancelled" label="Cancelled Woff Applications" />
          </Tabs>
        </CardContent>
      </Card>
