...
export default function EmployeeReports() {
  ...
  const exportOndutyMovement = () => {
    const groups = {}; filtered.forEach(r => { const k = String(r.status || "Pending").toLowerCase(); (groups[k] = groups[k] || []).push(r); });
    const head = ["App No", "Movement Date", "From", "To", "Status"]; const rows = []; Object.keys(groups).sort().forEach(k => { rows.push([{ content: `${k.toUpperCase()} — ${groups[k].length}`, colSpan: 5, styles: { fillColor: [232, 240, 254], textColor: [25, 103, 210], fontStyle: "bold" } }]); groups[k].forEach(r => rows.push([r.movement_id || r.id || "—", fmtDate(r.movement_date || r.act_date), r.perm_ftime || r.from_time || "—", r.perm_ttime || r.to_time || "—", r.status || "—"])); });
    const title = `OnDuty Movement — ${MONTHS[month - 1].label} ${year} — ${empid}`;
    if (reportType === "PDF") { const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "a4" }); doc.text(companyName || "Company", 420, 30, { align: "center" }); doc.text(title, 420, 45, { align: "center" }); autoTable(doc, { startY: 60, head: [head], body: rows.length ? rows : [["No records", "", "", "", ""]], styles: { fontSize: 7, cellPadding: 3 }, headStyles: { fillColor: [25, 118, 210] }, theme: "grid" }); doc.save(`OndutyMovement_${year}_${month}.pdf`); } else { const flat = []; Object.keys(groups).sort().forEach(k => { flat.push([`${k.toUpperCase()} — ${groups[k].length}`, "", "", "", ""]); groups[k].forEach(r => flat.push([r.movement_id || r.id || "—", fmtDate(r.movement_date || r.act_date), r.perm_ftime || r.from_time || "—", r.perm_ttime || r.to_time || "—", r.status || "—"])); }); const ws = XLSX.utils.aoa_to_sheet([[companyName || "Company"], [title], head, ...flat]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Onduty Movement"); XLSX.writeFile(wb, `OndutyMovement_${year}_${month}.xlsx`); }
  };
