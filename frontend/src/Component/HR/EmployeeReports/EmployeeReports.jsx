...
import { Box, Typography, Paper, Table, TableHead, TableRow, TableCell, TableBody, Chip, CircularProgress, TextField, Button, Dialog, DialogTitle, DialogContent, IconButton } from "@mui/material";
import { alpha, useTheme } from "@mui/material/styles";
import axios from "axios";
import { useLocation } from "react-router-dom";
import ModuleTabBar from "../common/ModuleTabBar";
import { FaCalendarCheck, FaUmbrellaBeach, FaMoneyBill, FaClock, FaFileAlt, FaDownload, FaEye } from "react-icons/fa";
...
const TabIcon = ({ icon }) => <span style={{ fontSize: 11 }}>{icon}</span>;
...
export default function EmployeeReports() {
  ...
  const [tab, setTab] = useState(() => new URLSearchParams(location.search).get("tab") || "attendance");
  ...
  const fmt = (v) => { const n = Number(v); return isNaN(n) ? "0" : n.toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 2 }); };

  const tabs = [
    { label: "My Attendance", value: "attendance", icon: <FaCalendarCheck /> },
    { label: "My Leaves", value: "leaves", icon: <FaUmbrellaBeach /> },
    { label: "My Payslips", value: "payslips", icon: <FaMoneyBill /> },
    { label: "My OT / Requests", value: "ot", icon: <FaClock /> },
  ];
  ...
  const fetchData = async () => {
    ...
    try {
      if (tab === "attendance") {
        const r = await axios.get(`${import.meta.env.VITE_API_URL}/api/attendance`, { params: { empid, month, year }, withCredentials: true });
        setData(r.data.records || r.data || []);
      } else if (tab === "leaves") {
        const r = await axios.get(`${import.meta.env.VITE_API_URL}/api/leave/my`, { params: { empid }, withCredentials: true }).catch(async () => await axios.get(`${import.meta.env.VITE_API_URL}/api/leave`, { params: { empid }, withCredentials: true }));
        setData(Array.isArray(r.data) ? r.data : r.data.records || []);
      } else if (tab === "payslips") {
        const r = await axios.get(`${import.meta.env.VITE_API_URL}/api/payroll/my-payslips`, { params: { empid }, withCredentials: true });
        setData(Array.isArray(r.data) ? r.data : r.data.payslips || r.data.records || []);
      } else if (tab === "ot") {
        const r = await axios.get(`${import.meta.env.VITE_API_URL}/api/attendance-requests/my`, { params: { empid }, withCredentials: true });
        setData(r.data || []);
      }
    } catch { setData([]); }
    setLoading(false);
  };
  useEffect(() => { const p = new URLSearchParams(location.search).get("tab"); if (p && ["attendance","leaves","payslips","ot"].includes(p)) setTab(p); }, [location.search]);
  ...
  const otherDed = (p) => Math.ceil(Number(p.C_DED_OTH)||0);
  const payslipHtml = (p) => {
    const emp = p.employee || {}; const off = emp.official || {};
    return `<html><head><title>Payslip_${p.C_EMPID}_${p.C_MONTH}_${p.C_YEAR}</title><style>body{font-family:Arial,sans-serif;padding:10px} .payslip-table{width:100%;border-collapse:collapse;font-size:11px} .payslip-table td{border:1px solid #aaa;padding:4px 6px} .payslip-bg-grey{background:#f5f5f5} .payslip-bold{font-weight:700} .payslip-text-center{text-align:center} .payslip-text-right{text-align:right} .payslip-logo-img{height:42px} .payslip-company-title{font-size:14px;text-align:center}</style></head><body>${document.getElementById('payslip-template-'+p.C_EMPID+'-'+p.C_MONTH)?.outerHTML || ''}<script>window.print()</script></body></html>`;
  };
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#f4f6f8" }}>
      <ModuleTabBar title="My Reports" tabs={tabs} active={tab} onChange={setTab} />
      <Box sx={{ p: 2, maxWidth: 1100, mx: "auto" }}>
        {tab === "attendance" && (
          <Paper elevation={0} sx={{ p: 1.5, mb: 1.5, border: "1px solid #e0e0e0", borderRadius: 1, display: "flex", gap: 1, alignItems: "center" }}>
            <TextField size="small" type="number" label="Month" value={month} onChange={e => setMonth(parseInt(e.target.value) || 1)} inputProps={{ min: 1, max: 12 }} sx={{ width: 90 }} />
            <TextField size="small" type="number" label="Year" value={year} onChange={e => setYear(parseInt(e.target.value) || 2026)} sx={{ width: 100 }} />
            <Button size="small" variant="contained" onClick={fetchData} sx={{ fontWeight: 600 }}>Load</Button>
            <Chip label={`${data.length} days`} size="small" sx={{ bgcolor: alpha(theme.palette.primary.main, 0.1), color: theme.palette.primary.main, fontWeight: 600 }} />
          </Paper>
        )}
        <Paper elevation={0} sx={{ border: "1px solid #e0e0e0", borderRadius: 1, overflow: "hidden" }}>
          {loading ? <Box sx={{ p: 4, textAlign: "center" }}><CircularProgress size={22} /></Box> :
            tab === "attendance" ? (
              <Table size="small"><TableHead><TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}><TableCell sx={{ fontWeight: 600 }}>Date</TableCell><TableCell sx={{ fontWeight: 600 }}>Status</TableCell><TableCell sx={{ fontWeight: 600 }}>In</TableCell><TableCell sx={{ fontWeight: 600 }}>Out</TableCell><TableCell sx={{ fontWeight: 600 }}>Late</TableCell><TableCell sx={{ fontWeight: 600 }}>OT</TableCell></TableRow></TableHead>
                <TableBody>{data.length === 0 ? <TableRow><TableCell colSpan={6} align="center" sx={{ py: 3, color: "#9e9e9e" }}>No records</TableCell></TableRow> : data.map((r, i) => (
                  <TableRow key={i} hover><TableCell sx={{ fontSize: 12 }}>{(r.att_date || r.attendance_date || "").toString().slice(0, 10).split("-").reverse().join("-")}</TableCell><TableCell><Chip size="small" label={r.status || r.att_status || "-"} sx={{ height: 18, fontSize: 10, fontWeight: 600 }} /></TableCell><TableCell sx={{ fontSize: 12 }}>{r.in_time ? r.in_time.slice(0, 5) : "-"}</TableCell><TableCell sx={{ fontSize: 12 }}>{r.out_time ? r.out_time.slice(0, 5) : "-"}</TableCell><TableCell sx={{ fontSize: 12 }}>{r.late_hrs ? parseFloat(r.late_hrs).toFixed(2) : "0.00"}</TableCell><TableCell sx={{ fontSize: 12 }}>{r.ot_hrs ? parseFloat(r.ot_hrs).toFixed(2) : "0.00"}</TableCell></TableRow>
                ))}</TableBody></Table>
            ) : tab === "leaves" ? (
              <Table size="small"><TableHead><TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}><TableCell sx={{ fontWeight: 600 }}>Date</TableCell><TableCell sx={{ fontWeight: 600 }}>Type</TableCell><TableCell sx={{ fontWeight: 600 }}>Status</TableCell><TableCell sx={{ fontWeight: 600 }}>Reason</TableCell></TableRow></TableHead>
                <TableBody>{data.length === 0 ? <TableRow><TableCell colSpan={4} align="center" sx={{ py: 3, color: "#9e9e9e" }}>No leaves</TableCell></TableRow> : data.map((r, i) => (
                  <TableRow key={i} hover><TableCell sx={{ fontSize: 12 }}>{(r.from_date || r.leave_date || r.applied_at || "").toString().slice(0, 10).split("-").reverse().join("-")}</TableCell><TableCell sx={{ fontSize: 12 }}>{r.leave_type || r.type || "-"}</TableCell><TableCell><Chip size="small" label={r.status || r.approval_status || "Pending"} sx={{ height: 18, fontSize: 10 }} /></TableCell><TableCell sx={{ fontSize: 12, maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.reason || "-"}</TableCell></TableRow>
                ))}</TableBody></Table>
            ) : tab === "payslips" ? (
              <><Table size="small"><TableHead><TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}><TableCell sx={{ fontWeight: 600 }}>Month/Year</TableCell><TableCell sx={{ fontWeight: 600 }}>Net Pay</TableCell><TableCell sx={{ fontWeight: 600 }}>Status</TableCell><TableCell sx={{ fontWeight: 600 }}>Actions</TableCell></TableRow></TableHead>
                <TableBody>{data.length === 0 ? <TableRow><TableCell colSpan={4} align="center" sx={{ py: 3, color: "#9e9e9e" }}>No payslips</TableCell></TableRow> : data.map((r, i) => (
                  <TableRow key={i} hover><TableCell sx={{ fontSize: 12 }}>{r.C_MONTH || r.month || r.pay_month || "-"} {r.C_YEAR || r.year || ""}</TableCell><TableCell sx={{ fontSize: 12, fontWeight: 600 }}>{r.C_NET_AMT ? `₹${Number(r.C_NET_AMT).toLocaleString("en-IN")}` : (r.NET_PAY || r.net_pay || "-")}</TableCell><TableCell><Chip size="small" label={r.C_FINAL_STATUS === 2 ? "Final" : r.status || "Generated"} sx={{ height: 18, fontSize: 10 }} /></TableCell><TableCell><Box sx={{ display: "flex", gap: 0.5 }}><Button size="small" variant="outlined" startIcon={<FaEye />} onClick={() => { setViewData(r); setViewOpen(true); }} sx={{ fontSize: 10, fontWeight: 600, minWidth: 0, px: 1 }}>Show</Button><Button size="small" variant="contained" startIcon={<FaDownload />} onClick={() => downloadPayslip(r)} sx={{ fontSize: 10, fontWeight: 600, minWidth: 0, px: 1 }}>Download</Button></Box></TableCell></TableRow>
                ))}</TableBody></Table>
                <Dialog open={viewOpen} onClose={() => setViewOpen(false)} maxWidth="md" fullWidth>
                  <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, fontWeight: 600 }}>Payslip — {viewData?.C_ENAME} ({viewData?.C_EMPID}) — {viewData?.C_MONTH} {viewData?.C_YEAR} <IconButton size="small" onClick={() => setViewOpen(false)}><CloseIcon /></IconButton></DialogTitle>
                  <DialogContent dividers sx={{ p: 1 }}>
                    {viewData && (() => { const p = viewData; const emp = p.employee || {}; const off = emp.official || {}; return (
                      <Box id="my-payslip-print" sx={{ fontSize: 11 }}>
                  ...
                  <Box sx={{ p: 1.2, display: "flex", justifyContent: "flex-end", gap: 1 }}><Button variant="contained" startIcon={<PrintIcon />} onClick={() => window.print()}>Print</Button><Button variant="contained" startIcon={<FaDownload />} onClick={() => downloadPayslip(viewData)}>Download / Print</Button><Button variant="outlined" onClick={() => setViewOpen(false)}>Close</Button></Box>
                </Dialog>
              </>
            ) : (
              <Table size="small"><TableHead><TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}><TableCell sx={{ fontWeight: 600 }}>Date</TableCell><TableCell sx={{ fontWeight: 600 }}>In/Out</TableCell><TableCell sx={{ fontWeight: 600 }}>Status</TableCell><TableCell sx={{ fontWeight: 600 }}>HR Status</TableCell></TableRow></TableHead>
                <TableBody>{data.length === 0 ? <TableRow><TableCell colSpan={4} align="center" sx={{ py: 3, color: "#9e9e9e" }}>No requests</TableCell></TableRow> : data.map(r => (
                  <TableRow key={r.id} hover><TableCell sx={{ fontSize: 12 }}>{r.att_date.slice(0, 10).split("-").reverse().join("-")}</TableCell><TableCell sx={{ fontSize: 12 }}>{r.in_time ? r.in_time.slice(0, 5) : "-"} - {r.out_time ? r.out_time.slice(0, 5) : "-"}</TableCell><TableCell><Chip size="small" label={r.status} sx={{ height: 18, fontSize: 10 }} /></TableCell><TableCell><Chip size="small" label={r.request_status} sx={{ height: 18, fontSize: 10, bgcolor: r.request_status === "Approved" ? "#e8f5e9" : r.request_status === "Pending" ? "#fff8e1" : "#ffebee", color: r.request_status === "Approved" ? "#2e7d32" : r.request_status === "Pending" ? "#ef6c00" : "#c62828", fontWeight: 600 }} /></TableCell></TableRow>
                ))}</TableBody></Table>
            )}
        </Paper>
        <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: "block" }}>Self-service reports • Data from live HR tables • Use filters above for month.</Typography>
      </Box>
    </Box>
  );
}
