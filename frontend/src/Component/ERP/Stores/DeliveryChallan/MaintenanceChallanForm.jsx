...
const MAINT_TYPES = ["Preventive", "Breakdown", "Corrective", "Predictive", "Routine"];
...
export default function MaintenanceChallanForm() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (id && isEdit) {
        ...
        showToast("Maintenance DC updated", "success");
      } else {
    ...
    <Box sx={{ p: 3, maxWidth: 1600 }}>
      ...
      <Card sx={{ borderRadius: "10px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", mb: 3, border: "1px solid #e2e8f0", overflow: "hidden" }}>
        <Box sx={{ px: 2.5, py: 1.2, bgcolor: "#f8fafc", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="subtitle2" sx={{ fontWeight: 700, fontSize: "0.85rem", color: "#1e293b", letterSpacing: "0.2px" }}>Maintenance Details</Typography>
        </Box>
        <CardContent sx={{ p: 2.5, "&:last-child": { pb: 2.5 } }}>
          <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap", alignItems: "flex-start" }}>
            <Box sx={{ width: 200 }}>
              <Autocomplete size="small" options={MAINT_TYPES} value={form.maintenance_type || null}
                onChange={(_, v) => setForm((f) => ({ ...f, maintenance_type: v || "" }))}
                onKeyDown={(e) => { onEnter(e); }}
                disabled={isView}
                renderInput={(p) => <TextField {...p} label="Maintenance Type" sx={fsx} />} />
            </Box>
            <Box sx={{ width: 220 }}>
            ...
            <Box sx={{ flex: "1 1 320px", minWidth: 280 }}>
              <TextField size="small" label="Instructions / Work to be Done" fullWidth multiline minRows={1} maxRows={3} value={form.remarks}
                onChange={handleChange("remarks")} onKeyDown={onEnter} disabled={isView} sx={fsx}
                placeholder="Special instructions, checklist or work to be carried out" />
            </Box>
          </Box>
        </CardContent>
      </Card>
