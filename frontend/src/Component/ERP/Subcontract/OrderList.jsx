...
export default function OrderList() {
  ...
  const columns = [
    ...
    { field: "order_date", headerName: "Order Date", width: 120, renderCell: (p) => formatDate(p.value) },
    { field: "expected_date", headerName: "Expected", width: 120, renderCell: (p) => formatDate(p.value) },
    ...
    { field: "actions", headerName: "Actions", width: 140, sortable: false, renderCell: (p) => (
      <Box>
        <Tooltip title="View"><IconButton size="small" onClick={() => navigate(`/subcontract/orders/view/${p.row.id}`)}><VisibilityIcon fontSize="small" /></IconButton></Tooltip>
        <Tooltip title="Edit"><IconButton size="small" onClick={() => navigate(`/subcontract/orders/edit/${p.row.id}`)}><EditIcon fontSize="small" /></IconButton></Tooltip>
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: "bold", color: "var(--heading-color)" }}>Job Work Orders</Typography>
