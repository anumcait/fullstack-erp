...
const STATUS_COLORS = {
  Draft: "default", Confirmed: "primary", Processing: "info",
...
export default function SalesOrderList() {
  ...
  const columns = [
    ...
    { field: "delivery_date", headerName: "Delivery", width: 110, renderCell: (p) => formatDate(p.value) },
    {
    ...
    { field: "items", headerName: "Items", width: 70, valueGetter: (v) => (v ? v.length : 0) },
    {
      ...
      renderCell: (p) => (
        <Box>
          <Tooltip title="View"><IconButton size="small" onClick={() => navigate(`/marketing/orders/view/${p.row.id}`)}><VisibilityIcon fontSize="small" /></IconButton></Tooltip>
          {p.row.status === "Draft" && (
            <Tooltip title="Edit"><IconButton size="small" onClick={() => navigate(`/marketing/orders/edit/${p.row.id}`)}><EditIcon fontSize="small" /></IconButton></Tooltip>
          )}
        </Box>
      ),
    },
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: "bold", color: "var(--heading-color)" }}>Sales Orders</Typography>
      <Card sx={{ borderRadius: 3, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <CardContent>
          <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate("/marketing/orders/add")}>New Order</Button>
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchData}>Refresh</Button>
          </Box>
          {loading && <LinearProgress sx={{ mb: 1 }} />}
          <div style={{ height: 520, width: "100%" }}>
            <StandardTable title="SalesOrderList" rows={rows} columns={columns} getRowId={(r) => r.id} pageSizeOptions={[10, 25, 50]}
