...
export default function PurchaseApprovals() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // filter only PR and PO types
            const rawItems = data.items || [];
            const purItems = rawItems.filter(item => ["PR", "PO"].includes(item.type));
            setRows(purItems);
        } catch {
            showToast("Failed to load purchase approvals", "error");
    ...
    const columns = [
        ...
        { field: "party", headerName: "Party / By", width: 220 },
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 3 } }}>
            <PageHeader
                ...
                subtitle="Purchase inbox — purchase requisitions and purchase orders awaiting your decision"
            ...
            <Stack direction="row" spacing={1} sx={{ mb: 2, flexWrap: "wrap", gap: 1 }}>
                {["ALL", ...Object.keys(TYPE_META)].map((t) => (
                    <Chip
                        key={t}
                        label={t === "ALL" ? `All (${rows.length})` : `${TYPE_META[t].label} (${counts[t] || 0})`}
                        onClick={() => setFilter(t)}
            ...
            <Card sx={{ borderRadius: 3, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
                <CardContent>
                    ...
                    <Box sx={{ height: 560, width: "100%" }}>
                        <StandardTable title="PurchaseApprovals"
