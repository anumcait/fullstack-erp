  ...
  useEffect(() => {
    if (id) {
      ...
      axios.get(`${API}/${id}`).then(({ data }) => setForm({
        ...
        machine_type: data.machine_type || "", department: data.department || "", location: data.location || "",
        manufacturer: data.manufacturer || "", model_no: data.model_no || "", serial_no: data.serial_no || "",
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <Card sx={{ borderRadius: 3, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
              <CardContent>
                <Typography variant="h6" sx={{ fontWeight: 600, mb: 2, color: "var(--heading-color)" }}>Specifications</Typography>
                <Divider sx={{ mb: 2 }} />
                <Grid container spacing={2}>
          ...
          {!isView && <Grid item xs={12}><Box display="flex" gap={2}>
            ...
            <Button variant="outlined" startIcon={<CancelIcon />} onClick={() => navigate("/maintenance/machines")}>Cancel</Button>
          </Box></Grid>}
        </Grid>
      </form>
    </Box>
