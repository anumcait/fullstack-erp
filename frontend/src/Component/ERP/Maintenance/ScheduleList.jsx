...
const STATUS_COLORS = { Pending: "warning", Overdue: "error", Completed: "success" };
...
export default function ScheduleList() {
  ...
  const columns = [
    ...
    { field: "last_done_date", headerName: "Last Done", width: 110, renderCell: (p) => formatDate(p.value) },
    { field: "next_due_date", headerName: "Next Due", width: 110, renderCell: (p) => formatDate(p.value) },
    ...
    { field: "actions", headerName: "Actions", width: 140, sortable: false, renderCell: (p) => (
      <Box>
        <Tooltip title="View"><IconButton size="small" onClick={() => navigate(`/maintenance/schedule/view/${p.row.id}`)}><VisibilityIcon fontSize="small" /></IconButton></Tooltip>
        <Tooltip title="Edit"><IconButton size="small" onClick={() => navigate(`/maintenance/schedule/edit/${p.row.id}`)}><EditIcon fontSize="small" /></IconButton></Tooltip>
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: "bold", color: "var(--heading-color)" }}>PM Schedule</Typography>
      <Card sx={{ borderRadius: 3, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <CardContent>
          <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap", alignItems: "center" }}>
            <TextField size="small" placeholder="Search by task name..." value={search} onChange={(e) => setSearch(e.target.value)}
              InputProps={{ startAdornment: <SearchIcon sx={{ mr: 1, color: "gray" }} /> }} sx={{ minWidth: 300 }} />
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate("/maintenance/schedule/add")}>New Schedule</Button>
