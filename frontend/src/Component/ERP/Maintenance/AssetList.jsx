...
const STATUS_COLORS = { Active: "success", Inactive: "default", Disposed: "error" };
...
export default function AssetList() {
  ...
  const columns = [
    ...
    { field: "purchase_date", headerName: "Purch Date", width: 110, renderCell: (p) => formatDate(p.value) },
    { field: "purchase_cost", headerName: "Cost", width: 110 },
    { field: "warranty_expiry", headerName: "Warranty", width: 110, renderCell: (p) => formatDate(p.value) },
    ...
    { field: "actions", headerName: "Actions", width: 140, sortable: false, renderCell: (p) => (
      <Box>
        <Tooltip title="View"><IconButton size="small" onClick={() => navigate(`/maintenance/assets/view/${p.row.id}`)}><VisibilityIcon fontSize="small" /></IconButton></Tooltip>
        <Tooltip title="Edit"><IconButton size="small" onClick={() => navigate(`/maintenance/assets/edit/${p.row.id}`)}><EditIcon fontSize="small" /></IconButton></Tooltip>
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3, fontWeight: "bold", color: "var(--heading-color)" }}>Asset Register</Typography>
      <Card sx={{ borderRadius: 3, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
        <CardContent>
          <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate("/maintenance/assets/add")}>New Asset</Button>
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchData}>Refresh</Button>
          </Box>
          {loading && <LinearProgress sx={{ mb: 1 }} />}
          <div style={{ height: 520, width: "100%" }}>
            <StandardTable title="AssetList" rows={rows} columns={columns} getRowId={(r) => r.id} pageSizeOptions={[10, 25, 50]}
